        ...
        {adding && (
          <div className="rounded-lg border border-line px-3 py-2.5 space-y-2">
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => { setAdding(false); setError(""); }} disabled={busy} className="text-[13px] text-muted hover:bg-canvas rounded-lg px-3 py-1.5">
                Cancel
              </button>
              <button onClick={add} disabled={busy || !form.name.trim()} className="text-[13px] font-semibold bg-primary hover:bg-primary-hover text-white rounded-lg px-3 py-1.5 disabled:opacity-60">
                {busy ? "Saving…" : "Add"}
              </button>
            </div>
          </div>
        ...
        {employees.map((e) => (
          <div key={e.id} className="flex items-center gap-3 rounded-lg border border-line px-3 py-2.5">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[12px] text-faint">RM {fmt2(e.basicSalary)}/month{e.epfNo ? ` · EPF ${e.epfNo}` : ""}</div>
            </div>
            <button
