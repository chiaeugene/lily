...
async function callClaude(message: string): Promise<ParsedDraft | null> {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) return null;
  const model = process.env.CLAUDE_PARSER_MODEL || "claude-haiku-4-5-20251001";
  const products = repo.listProducts().map((p) => `${p.name} (uom ${p.uom})`).join("; ");
  const customers = repo.listCustomers().map((c) => c.name).join("; ");
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "user",
          content: `Known products: ${products}\nKnown customers: ${customers}\n\nMessage:\n${message}`,
        },
      ],
    }),
  });
...
function heuristic(message: string): ParsedDraft {
  ...
  const cust = repo.listCustomers().find((c) => m.includes(c.name.toLowerCase()));
  const prod = repo.listProducts().find((p) =>
